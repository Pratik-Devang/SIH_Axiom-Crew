...
def main() -> None:
    ...
    time_col = "time_since_start_s" if "time_since_start_s" in trip.columns else "timestamp_s"
    if time_col in trip.columns:
        dt = trip[time_col].diff().dropna()
        print(f"smartphone dt seconds min/median/mean/max: {dt.min():.3f}, {dt.median():.3f}, {dt.mean():.3f}, {dt.max():.3f}")
    if "sample_period_s" in trip.columns:
        print(f"vehicle sample period unique: {sorted(trip['sample_period_s'].unique())[:5]}")
    print("alignment: synchronized 10 Hz resampled dataset")
    print()
