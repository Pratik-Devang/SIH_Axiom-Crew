...
To prevent the model from memorizing specific trips (overfitting), the canonical
split is explicitly listed in `data/splits/io_vnbd_splits.yaml`. Complete trip
families are assigned to exactly one of train, validation, or test. Training now
fails if a trip is duplicated, omitted, or unknown. Counts and metrics must be
read from the `model_info.json` and `speed_metrics.json` generated by the same
`run_id`; previously committed counts describe a legacy run and are not current.
...
### Q12: Why do we use an explicit family-disjoint trip manifest?
**A**: Random window splitting leaks nearly identical samples across sets, while
filename-order splitting is implicit and can change as files are added. The
manifest keeps related trip variants together and makes every evaluation split
reviewable and reproducible. A genuinely unseen phone/vehicle field dataset is
still required before claiming broad out-of-distribution performance.
