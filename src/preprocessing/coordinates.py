...
def add_local_coordinates(
    ...
    """Add local East/North coordinates to an entire dataframe.

    The first row containing both a valid latitude and longitude defines the
    local ENU origin. Existing geographic columns are preserved, and rows with
    missing geographic coordinates receive missing East/North values.

    This dataframe-oriented adapter retains the IO-VNBD preprocessing API while
    using the same WGS84 curvature model as :func:`latlon_to_enu`.
    """
    result = df.copy()
    ...
    origin = LatLonOrigin(
        lat=float(latitude.loc[valid].iloc[0]),
        lon=float(longitude.loc[valid].iloc[0]),
    )
    lat0_rad = math.radians(origin.lat)
    meridional_radius, prime_vertical_radius = _radius_of_curvature(lat0_rad)
