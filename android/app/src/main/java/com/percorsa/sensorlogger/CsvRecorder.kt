...
data class CsvNavigationDiagnostics(
    ...
    val tcnRejectionReason: String? = null,
    val eskfInitialized: Boolean? = null,
    val eskfValid: Boolean? = null,
    val eskfRuntimeState: String? = null,
    val eskfDegradationReason: String? = null,
    val eskfCalibrationActive: Boolean? = null,
...
open class CsvRecorder(context: Context? = null, overrideFile: File? = null) {
    ...
    val file: File
    /** Lossless raw callback stream; the navigation CSV remains in [file]. */
    val rawImuFile: File
    private var writer: BufferedWriter? = null
    private var rawImuWriter: BufferedWriter? = null
    ...
    init {
        ...
        rawImuFile = File(
            ...
            "${file.nameWithoutExtension}_raw_imu.csv"
        )
    ...
    private fun initWriter() {
        try {
            ...
            rawImuWriter = BufferedWriter(FileWriter(rawImuFile, true))
            writer?.write(
                ...
                "tcn_canonical_timestamp_ns,tcn_inference_active,tcn_raw_speed_mps,tcn_filtered_speed_mps,tcn_rate_limited,tcn_rejected_count,vehicle_motion_observed,tcn_injected_into_ins,tcn_accepted_by_eskf,tcn_nis,tcn_rejection_reason," +
                "eskf_initialized,eskf_valid,eskf_runtime_state,eskf_degradation_reason,eskf_calibration_active,eskf_timestamp_ns,eskf_dt_s,eskf_latitude,eskf_longitude,eskf_position_east_m,eskf_position_north_m,eskf_position_up_m," +
                ...
                "eskf_covariance_trace,eskf_state_finite,eskf_covariance_finite,eskf_covariance_psd,eskf_gnss_accepted,eskf_gnss_nis,eskf_gnss_innovation_m,eskf_stationary,eskf_nhc_enabled,eskf_nhc_accepted,eskf_nhc_nis,eskf_zupt_enabled,eskf_zupt_accepted," +
                "route_segment_index,route_progress_m,route_lateral_error_m,route_heading_error_deg,turn_state,turn_yaw_rate_deg_s,off_route,rerouting\n"
            )
            rawImuWriter?.write(
                "session_id,raw_event_index,timestamp_ns,sensor_type,accel_x,accel_y,accel_z,gyro_x,gyro_y,gyro_z\n"
            )
            writer?.flush()
            rawImuWriter?.flush()
        } catch (e: Exception) {
    ...
    private var rawEventIndex: Long = 0L

    /**
     * Append one immutable raw Android sensor callback to the sidecar stream.
     * Only the channel belonging to [sensorType] is populated; other channels
     * are explicitly missing rather than represented as zero.
     */
    @Synchronized
    fun writeRawImuEvent(
        sensorType: String,
        timestampNs: Long,
        accelX: Float = Float.NaN,
        accelY: Float = Float.NaN,
        accelZ: Float = Float.NaN,
        gyroX: Float = Float.NaN,
        gyroY: Float = Float.NaN,
        gyroZ: Float = Float.NaN
    ) {
        val index = rawEventIndex++
        val row = listOf(
            sessionId,
            index.toString(),
            timestampNs.toString(),
            sensorType,
            csvNumber(accelX), csvNumber(accelY), csvNumber(accelZ),
            csvNumber(gyroX), csvNumber(gyroY), csvNumber(gyroZ)
        ).joinToString(",") + "\n"
        val runnable = Runnable {
            try {
                rawImuWriter?.write(row)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
        if (handler != null) handler?.post(runnable) else runnable.run()
    }
    ...
    open fun writeRow(
        ...
        // Capture every mutable diagnostic value at the producer timestamp.
        // The writer thread must never associate a later state with this row.
        val capturedEstimatedSpeed = estimatedSpeedMps
        val capturedTcnSpeed = tcnSpeedMps
        val capturedRawAccelTimestampNs = rawAccelTimestampNs
        val capturedRawGyroTimestampNs = rawGyroTimestampNs
        val capturedGnssTimestampMs = gnssTimestampMs
        val capturedGnssElapsedRealtimeNs = gnssElapsedRealtimeNs
        val capturedGnssAltitudeM = gnssAltitudeM
        val capturedDiagnostics = navigationDiagnostics
        val runnable = Runnable {
            try {
                ...
                val estimatedSpeed = capturedEstimatedSpeed
                val tcnSpeed = capturedTcnSpeed

                writer?.let { w ->
                    val d = capturedDiagnostics
                    ...
                    val altitudeStr = if (capturedGnssAltitudeM.isNaN()) "" else "%.3f".format(Locale.US, capturedGnssAltitudeM)
                    ...
                    val value = listOf(
                        sessionId, timestampNs.toString(), "%.4f".format(Locale.US, timeSinceStartS), capturedRawAccelTimestampNs.takeIf { it > 0L }?.toString() ?: "", capturedRawGyroTimestampNs.takeIf { it > 0L }?.toString() ?: "",
                        ...
                        capturedGnssTimestampMs.takeIf { it > 0L }?.toString() ?: "", capturedGnssElapsedRealtimeNs.takeIf { it > 0L }?.toString() ?: "",
                        ...
                        d.tcnCanonicalTimestampNs.takeIf { it > 0L }?.toString() ?: "", csvBoolean(d.tcnInferenceActive), csvNumber(d.tcnRawSpeedMps), csvNumber(d.tcnFilteredSpeedMps), csvBoolean(d.tcnPredictionRateLimited), d.tcnRejectedPredictionCount?.toString() ?: "", csvBoolean(d.vehicleMotionObserved), csvBoolean(d.tcnInjectedIntoIns), csvBoolean(d.tcnAcceptedByEskf), csvNumber(d.tcnNis), d.tcnRejectionReason ?: "",
                        csvBoolean(d.eskfInitialized), csvBoolean(d.eskfValid), d.eskfRuntimeState ?: "", d.eskfDegradationReason ?: "", csvBoolean(d.eskfCalibrationActive), d.eskfTimestampNs.takeIf { it > 0L }?.toString() ?: "", csvNumber(d.eskfDtSeconds), csvNumber(d.eskfPositionLatitude), csvNumber(d.eskfPositionLongitude), csvNumber(d.eskfPositionEastM), csvNumber(d.eskfPositionNorthM), csvNumber(d.eskfPositionUpM),
                        csvNumber(d.eskfVelocityEastMps), csvNumber(d.eskfVelocityNorthMps), csvNumber(d.eskfVelocityUpMps), csvNumber(d.eskfSpeedMps), csvNumber(d.eskfHeadingDeg), csvNumber(d.eskfQuaternionW), csvNumber(d.eskfQuaternionX), csvNumber(d.eskfQuaternionY), csvNumber(d.eskfQuaternionZ), csvNumber(d.eskfQuaternionNorm), csvNumber(d.eskfCovarianceTrace), csvBoolean(d.eskfStateFinite), csvBoolean(d.eskfCovarianceFinite), csvBoolean(d.eskfCovariancePsd), csvBoolean(d.eskfGnssAccepted), csvNumber(d.eskfGnssNis), csvNumber(d.eskfGnssInnovationM), csvBoolean(d.eskfStationary), csvBoolean(d.eskfNhcEnabled), csvBoolean(d.eskfNhcAccepted), csvNumber(d.eskfNhcNis), csvBoolean(d.eskfZuptEnabled), csvBoolean(d.eskfZuptAccepted), d.routeSegmentIndex?.toString() ?: "", csvNumber(d.routeProgressM), csvNumber(d.routeLateralErrorM), csvNumber(d.routeHeadingErrorDeg), d.turnState ?: "", csvNumber(d.turnYawRateDegS), csvBoolean(d.offRoute), csvBoolean(d.rerouting)
                    ).joinToString(",")
                    w.write("$value\n")
                }
            } catch (e: Exception) {
